...
async def create_conversational_devops_ai(config_key: str = "devops_agent") -> RealMCPDevOpsAgent:
    """Create and initialize real MCP DevOps AI"""

    # Create agent config for DevOps
    config = AgentConfig(
        name="DevOps AI Assistant",
        bot_token=os.getenv("DISCORD_TOKEN_DEVOPS", ""),
        server_id=os.getenv("DEFAULT_SERVER_ID", ""),
        api_key=os.getenv("OPENAI_API_KEY", ""),  # Use OpenAI for conversations
        llm_type="openai",
    ...
    # Create and initialize the real MCP AI
    ai = RealMCPDevOpsAgent(config)

    # Ensure memory connection
    if not ai.memory.client.pool:
        await ai.memory.client.connect()
...
async def main():
    """Main function for running real MCP DevOps AI standalone"""

    ai = await create_conversational_devops_ai()

    logger.info("🚀 Starting Real MCP DevOps AI...")

    try:
        await ai.run()
    except KeyboardInterrupt:
        logger.info("👋 Real MCP DevOps AI shutdown requested")
    except Exception as e:
        logger.error(f"💥 Real MCP DevOps AI error: {e}")
        raise
